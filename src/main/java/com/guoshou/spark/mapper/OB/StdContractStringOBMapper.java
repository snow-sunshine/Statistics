...
@Repository
public interface StdContractStringOBMapper extends BaseMapper<StdContractString> {

    //期末保有 End of term retention
    @DS("OB")
    @Select("select count(*) from cl_cntr5_std_contract where polCode = #{polCode} ")
    long selEndOfTermRetention(@Param("polCode") String polCode);


    @DS("OB")
    @Select("select count(*) from cl_cntr5_std_contract where polCode = #{polCode} and cntrStat='K' and STR_TO_DATE(cntrExpiryDate, '%Y-%m-%d')>#{endTime}")
    long selEndNorCountA(@Param("polCode") String polCode,@Param("endTime") String endTime);

    //期末满期
    @DS("OB")
    @Select("SELECT COUNT(DISTINCT a.cntrNo) \n" +
            "FROM cl_cntr5_std_contract a\n" +
            "JOIN cl_biz5_endow_acc b ON a.cntrNo = b.accNo\n" +
    ...
    @DS("OB")
    @Select("SELECT count(distinct(a.cntrNo))  " +
            "FROM cl_cntr5_std_contract a, cl_biz5_plnmio_rec b " +
            " WHERE a.polCode= #{polCode} " +
            "AND a.cntrStat='T' " +
            "AND a.cntrTermCause='7' " +
    ...
    @Select("SELECT count(b.cntrNo) \n" +
            "FROM cl_biz5_svr_group a, cl_biz5_plnmio_rec b\n" +
            "WHERE a.polCode=#{polCode}\n" +
            "AND a.cntrStat='T'\n" +
            "AND a.cntrTermCause='7'\n" +
            "and a.sgNo=b.cntrNo;")
    ...
    @DS("OB")
    //期末失效
    @Select("SELECT count(*) FROM cl_cntr5_std_contract WHERE polCode=#{polCode} AND  cntrStat='L'")
    ...
    @DS("OB")
    @Select("SELECT count(distinct(a.cntrNo)) \n" +
            "FROM cl_cntr5_std_contract a , cl_biz5_mtn_alt_cntr b\n" +
            "WHERE a.polCode=#{polCode}\n" +
    ...
    @DS("OB")
    @Select("SELECT count(distinct(a.cntrNo))  \n" +
            "FROM cl_cntr5_std_contract a, cl_biz5_endow_acc b\n" +
            "WHERE a.polCode=#{polCode}\n" +
            "AND a.cntrStat='T'\n" +
            "AND a.cntrTermCause='7'\n" +
    ...
    //---------------------------jituan   b4-------------------------------------------
    @DS("OB")
    @Select("SELECT inForceDate FROM cl_cntr5_std_contract WHERE cntrNo=#{cntrNo}")
    StdContractString getInForceDate(@Param("cntrNo") String cntrNo);

    @DS("OB")
    @Select("SELECT faceFmnt FROM cl_cntr5_cntr_sub_state WHERE cntrId in (SELECT cntrId FROM cl_cntr5_std_contract WHERE cntrNo=#{cntrNo})")
