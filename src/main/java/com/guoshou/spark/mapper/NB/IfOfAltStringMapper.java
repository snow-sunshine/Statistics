...
import com.guoshou.spark.po.JavaNB.IfOfAltString;
...
@Repository
public interface IfOfAltStringMapper extends BaseMapper<IfOfAltString> {

    //待优化
//    @Select("SELECT count(*)\n" +
//            "FROM cl_cntr4_if_of_alt\n" +
//            "WHERE cntrId in (select cntrId from cl_cntr4_std_contract where polCode=#{polCode})\n" +
//            "AND STR_TO_DATE(outForceDate, '%Y-%m-%d') <= STR_TO_DATE(reInForceDate, '%Y-%m-%d')\n" +
//            "AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <>'9999-12-31'  AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <> #{startTime}\n" +
//            "AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <> #{endTime}")
    @Select("SELECT count(*)" +
            ...
            " AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <= #{endTime}")
    //select count(*) FROM cl_cntr4_if_of_alt WHERE cntrId in (select cntrId from cl_cntr4_std_contract where polCode=#{polCode})
    //AND STR_TO_DATE(outForceDate, '%Y-%m-%d') < STR_TO_DATE(reInForceDate, '%Y-%m-%d')
    // AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <> '9999-12-31' AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') >=''
    //and STR_TO_DATE(reInForceDate, '%Y-%m-%d') <=
...
@Select("SELECT COUNT(*) \n" +
    ...
    @Select("SELECT count(*) " +
            "FROM cl_cntr4_if_of_alt " +
            "WHERE cntrId in (select cntrId from cl_cntr4_std_contract where polCode=#{polCode} and mgrBranchNo=#{branchNo}) " +
            "AND STR_TO_DATE(outForceDate, '%Y-%m-%d') < STR_TO_DATE(reInForceDate, '%Y-%m-%d') " +
            "AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <> '9999-12-31' " +
            "AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') >= #{startTime} " +
            "AND STR_TO_DATE(reInForceDate, '%Y-%m-%d') <= #{endTime}")
